...
!> functions (cuda.h, CUDA 12.4+), used by mod_streams_ff.f90 to give
...
!> Validated standalone before integration -- see scratchpad/green_ctx_prototype/
!> from the Stage 1 spike (compiles, links -lcuda, correct struct layout,
!> correct uneven-split "remaining" pattern, plain <<<...,stream>>> launch
!> syntax works unmodified against a green-context-derived stream).
!>
