...
- **Arguments & Variables:**
  - `id_scheme` (`integer(2), intent(in), value`)
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `ruvwp(5,nx,ny,nz), T(nx,ny,nz)` (`real(8), intent(in), device`)
...
- **Arguments & Variables:**
  - `id_scheme` (`real(2), intent(in), value`)
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `ruvwp(5,nx,ny,nz), T(nx,ny,nz)` (`real(8), intent(in), device`)
  - `E(5,nx-1,ny-2,nz-2)` (`real(8), intent(out), device`)
  - `F(5,nx-2,ny-1,nz-2)` (`real(8), intent(out), device`)
...
- **Arguments & Variables:**
  - `id_scheme` (`real(4), intent(in), value`)
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `ruvwp(5,nx,ny,nz), T(nx,ny,nz)` (`real(8), intent(in), device`)
  - `E(5,nx-1,ny-2,nz-2)` (`real(8), intent(out), device`)
  - `F(5,nx-2,ny-1,nz-2)` (`real(8), intent(out), device`)
...
- **Arguments & Variables:**
  - `id_scheme` (`real(8), intent(in), value`)
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `ruvwp(5,nx,ny,nz), T(nx,ny,nz)` (`real(8), intent(in), device`)
  - `E(5,nx-1,ny-2,nz-2)` (`real(8), intent(out), device`)
  - `F(5,nx-2,ny-1,nz-2)` (`real(8), intent(out), device`)
...
### `subroutine calc_EFG_Euler(id_visc, nx, ny, nz, dx, dy, dz, Jacobian, QJ, ruvwp, T, mu, mut, qc2, E, F, G)`
- **Arguments & Variables:**
  - `id_visc` (`integer(kind=2), intent(in), value`)
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `Jacobian(nx,ny)` (`real(8), intent(in), device`)
  - `T(nx,ny,nz), mu(1,1,1), mut(1,1,1), qc2(1,1,1)` (`real(8), intent(out), device`)
  - `E(5,nx-1,ny-2,nz-2)` (`real(8), intent(out), device`)
  - `F(5,nx-2,ny-1,nz-2)` (`real(8), intent(out), device`)
  - `G(5,nx-2,ny-2,nz-1)` (`real(8), intent(out), device`)

### `subroutine calc_EFG_visc(id_visc, nx, ny, nz, dx, dy, dz, Jacobian, QJ, ruvwp, T, mu, mut, qc2, E, F, G)`
- **Arguments & Variables:**
  - `id_visc` (`integer(kind=4), intent(in), value`)
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `Jacobian(nx,ny)` (`real(8), intent(in), device`)
  - `T(nx,ny,nz), mu(nx,ny,nz), mut(1,1,1), qc2(1,1,1)` (`real(8), intent(out), device`)
  - `E(5,nx-1,ny-2,nz-2)` (`real(8), intent(out), device`)
  - `F(5,nx-2,ny-1,nz-2)` (`real(8), intent(out), device`)
  - `G(5,nx-2,ny-2,nz-1)` (`real(8), intent(out), device`)

### `subroutine calc_EFG_LES(id_visc, nx, ny, nz, dx, dy, dz, Jacobian, QJ, ruvwp, T, mu, mut, qc2, E, F, G)`
- **Arguments & Variables:**
  - `id_visc` (`integer(kind=8), intent(in), value`)
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `Jacobian(nx,ny)` (`real(8), intent(in), device`)
  - `T(nx,ny,nz), mu(nx,ny,nz), mut(nx,ny,nz), qc2(nx,ny,nz)` (`real(8), intent(out), device`)
  - `E(5,nx-1,ny-2,nz-2)` (`real(8), intent(out), device`)
  - `F(5,nx-2,ny-1,nz-2)` (`real(8), intent(out), device`)
  - `G(5,nx-2,ny-2,nz-1)` (`real(8), intent(out), device`)
...
- **Arguments & Variables:**
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `dx` (`real(8), intent(in), dimension(nx-1), device`)
  - `dy` (`real(8), intent(in), dimension(ny-1), device`)
  - `dz` (`real(8), intent(in), dimension(nz-1), device`)
  - `E` (`real(8), intent(in), dimension(5,nx-1,ny-2,nz-2), device`)
  - `F` (`real(8), intent(in), dimension(5,nx-2,ny-1,nz-2), device`)
  - `G` (`real(8), intent(in), dimension(5,nx-2,ny-2,nz-1), device`)
  - `R` (`real(8), intent(out), dimension(5,nx-2,ny-2,nz-2),device`)

### `subroutine calc_step1(nx, ny, nz, coef, dx, dy, dz, E, F, G, Q, Q2)`
- **Arguments & Variables:**
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `coef` (`real(8), intent(in), value`)
  - `dx` (`real(8), intent(in), dimension(nx-1), device`)
  - `dy` (`real(8), intent(in), dimension(ny-1), device`)
  - `dz` (`real(8), intent(in), dimension(nz-1), device`)
  - `E` (`real(8), intent(in), dimension(5,nx-1,ny-2,nz-2), device`)
  - `F` (`real(8), intent(in), dimension(5,nx-2,ny-1,nz-2), device`)
  - `G` (`real(8), intent(in), dimension(5,nx-2,ny-2,nz-1), device`)
  - `Q` (`real(8), intent(in), dimension(5,nx,ny,nz), device`)
  - `Q2` (`real(8), intent(out), dimension(5,nx,ny,nz), device`)

### `subroutine calc_step(nx, ny, nz, coef1, coef2, dx, dy, dz, E, F, G, Q, Q2, Rs)`
- **Arguments & Variables:**
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `coef1, coef2` (`real(8), intent(in), value`)
  - `dx` (`real(8), intent(in), dimension(nx-1), device`)
  - `dy` (`real(8), intent(in), dimension(ny-1), device`)
  - `dz` (`real(8), intent(in), dimension(nz-1), device`)
  - `E` (`real(8), intent(in), dimension(5,nx-1,ny-2,nz-2), device`)
  ...
  - `Q` (`real(8), intent(in), dimension(5,nx,ny,nz), device`)
  - `Q2` (`real(8), intent(out), dimension(5,nx,ny,nz), device`)
  - `Rs` (`real(8), intent(inout), dimension(5,nx-2,ny-2,nz-2), device`)

### `subroutine calc_step2_3(nx, ny, nz, coef1, coef2, coef3, coef4, dx, dy, dz, E, F, G, Qin, Qout)`
- **Arguments & Variables:**
  - `nx, ny, nz` (`integer, intent(in), value`)
  - `coef1, coef2, coef3, coef4` (`real(8), intent(in), value`)
  - `dx` (`real(8), intent(in), dimension(nx-1), device`)
  - `dy` (`real(8), intent(in), dimension(ny-1), device`)
  - `dz` (`real(8), intent(in), dimension(nz-1), device`)
  - `E` (`real(8), intent(in), dimension(5,nx-1,ny-2,nz-2), device`)
  - `F` (`real(8), intent(in), dimension(5,nx-2,ny-1,nz-2), device`)
  - `G` (`real(8), intent(in), dimension(5,nx-2,ny-2,nz-1), device`)
  - `Qin` (`real(8), intent(in), dimension(5,nx,ny,nz), device`)
  - `Qout` (`real(8), intent(inout), dimension(5,nx,ny,nz), device`)
...
- **Arguments & Variables:**
  ...
  - `G` (`real(8), intent(in), dimension(5,nx-2,ny-2,nz-1), device`)
  - `Rs` (`real(8), intent(inout), dimension(5,nx-2,ny-2,nz-2), device`)
  - `Q` (`real(8), intent(inout), dimension(5,nx,ny,nz), device`)
...
- **Arguments & Variables:**
  ...
  - `sx = threadsF%x` (`integer, parameter`)
  - `sy = threadsF%y + 5` (`integer, parameter`)
  - `sz = threadsF%z` (`integer, parameter`)
  - `rho, u, v, w, p, tmp` (`real(8), dimension(-1:sx*sy*sz-2), shared`)
...
- **Arguments & Variables:**
  ...
  - `sx = threadsG%x` (`integer, parameter`)
  - `sy = threadsG%y` (`integer, parameter`)
  - `sz = threadsG%z + 5` (`integer, parameter`)
  - `rho, u, v, w, p, tmp` (`real(8), dimension(-1:sx*sy*sz-2), shared`)
...
- **Arguments & Variables:**
  ...
  - `T(nx,ny,nz)` (`real(8), intent(in), device`) : Temperature
  - `E(5,nx-1,ny-2,nz-2)` (`real(8), intent(out), device`) : Flux in x direction
  - `sx = threadsE%x + 3` (`integer, parameter`)
  - `sy = threadsE%y` (`integer, parameter`)
  - `sz = threadsE%z` (`integer, parameter`)
...
- **Arguments & Variables:**
  ...
  - `F(5,nx-2,ny-1,nz-2)` (`real(8), intent(out), device`) : Flux in y direction
  - `sx = threadsF%x` (`integer, parameter`)
  - `sy = threadsF%y + 3` (`integer, parameter`)
  - `sz = threadsF%z` (`integer, parameter`)
  - `rho, u, v, w, p, tmp` (`real(8), dimension(0:sx*sy*sz-1), shared`)
...
- **Arguments & Variables:**
  ...
  - `sx = threadsG%x` (`integer, parameter`)
  - `sy = threadsG%y` (`integer, parameter`)
  - `sz = threadsG%z + 3` (`integer, parameter`)
  - `rho, u, v, w, p, tmp` (`real(8), dimension(0:sx*sy*sz-1), shared`)
