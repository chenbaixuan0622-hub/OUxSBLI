...
contains
  ...
  attributes(global) subroutine calc_Ducros_curv(nx, ny, nz, dz, xi_x, xi_y, eta_x, eta_y, Q, fd)
    ...
    dudxi  = 0.5d0 * (Q(i+1,2,j,k) - Q(i-1,2,j,k))
    dvdxi  = 0.5d0 * (Q(i+1,3,j,k) - Q(i-1,3,j,k))
    dwdxi  = 0.5d0 * (Q(i+1,4,j,k) - Q(i-1,4,j,k))
    dudeta = 0.5d0 * (Q(i,2,j+1,k) - Q(i,2,j-1,k))
    dvdeta = 0.5d0 * (Q(i,3,j+1,k) - Q(i,3,j-1,k))
    dwdeta = 0.5d0 * (Q(i,4,j+1,k) - Q(i,4,j-1,k))
    dudz   = 0.5d0 * (Q(i,2,j,k+1) - Q(i,2,j,k-1)) / dz
    dvdz   = 0.5d0 * (Q(i,3,j,k+1) - Q(i,3,j,k-1)) / dz
    dwdz   = 0.5d0 * (Q(i,4,j,k+1) - Q(i,4,j,k-1)) / dz
