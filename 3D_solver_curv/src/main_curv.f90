program main
  ...
  ! Convert to Jacobian_code = 1/(J_2D*dz) as required by pre_calc_curv convention
  ! (QJ = Q_physical * J_2D * dz; dt_Szeta = dt * J_2D).
  do j = 1, ny
    do i = 1, nx
  ...
  if (mod(myrank,2) == 0) then
    if (kind(id_recal) == 4) then
      ...
      if (ios /= 0) then
        ...
        call MPI_ABORT(MPI_COMM_WORLD, errorcode, ierr); stop
      endif
      ...
      if (is_sequential) then
        ...
        read(10) header; read(10) Q
        print *, "myrank is ", myrank, "simulation has been restarted (sequential)"
      else
        open(10, file=filename, action="read", form="unformatted", access="stream", status="old")
        rewind(10); read(10) Q
        print *, "myrank is ", myrank, "simulation has been restarted (stream)"
      endif
      close(10)
  ...
  if (mod(myrank,2) == 0) then
    ...
    ! save restart file: multiply QJ back by Jacobian_code to get Q_physical
    do l = 1, nz
      do j = 1, ny
        do i = 1, nx
          do m = 1, 5
