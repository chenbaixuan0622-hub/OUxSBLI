...
Qk4_directory = "../../../../../media/user/HD-EDS-E/hatayama/TBL/TBL20240826_KEEP4thVisc2nd"
...
pf_path = os.path.join(Qk4_directory, "pf.npy")
# pf[space, time]
pf = np.load(pf_path)
freq, psd = PSD(pf, t, num_files)

save_path = os.path.join(Qk4_directory, "PSD_p.d")
with open(save_path, "w", encoding="UTF-8") as f:
  print("# k       PSD", file=f)
  for i in range(1,int(0.5*num_files)):
    print(f'{freq[i]:.3e}', f'{psd[i]:.3e}', file=f)
