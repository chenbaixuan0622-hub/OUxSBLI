...
file_path = os.path.join(Q_directory, "Q00300.vtr")
...
Mt = np.mean(ut / np.sqrt(gamma * Q[4,:,0,:] / Q[0,:,0,:]))
...
mu = Sutherland(Q[4,:,0,:] / (Rgas * Q[0,:,0,:]))
save_path = os.path.join(Q_directory, "wall_unit.d")
with open(save_path, "w", encoding="UTF-8") as f:
  print("# rhow       ut         mu       tau       Mt", file=f)
  print(f'{np.mean(Q[0,:,0,:]):.3e}', f'{utm:.3e}', f'{np.mean(mu):.3e}', f'{twm:.3e}', f'{Mt:.3e}', file=f)
