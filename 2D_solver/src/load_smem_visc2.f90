...
contains
  attributes(device) subroutine load_smem_visc2_x(it, jt, j, idx, nx, ny, Q, u, v)
    ...
    do ii = it, threadsEv%x+1, blockDim%x
      ...
      if (1 <= i .and. i <= nx .and. j <= ny) then
        call pipelineMemcpyAsync(u(idx_l), Q(i,2,j))
        call pipelineMemcpyAsync(v(idx_l), Q(i,3,j))
      else
        u(idx_l) = 0.d0
        v(idx_l) = 0.d0
      endif
  ...
  attributes(device) subroutine load_smem_visc2_y(it, jt, i, idx, nx, ny, Q, u, v)
    integer, intent(in), value              :: it         !< local idx for x direction
    integer, intent(in), value              :: jt         !< local idx for y direction
