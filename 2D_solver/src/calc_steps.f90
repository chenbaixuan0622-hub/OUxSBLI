...
contains
  !$dir inline
  attributes(device) subroutine calc_R(nx, ny, i, j, dtdx, dtdy, E, F, R)      !dtdx, dtdy?
    ...
    real(8), intent(in), value              :: dtdx             !< dt * Sx
    real(8), intent(in), value              :: dtdy             !< dt * Sy
    ...
    R(2) = dtdy * (-E(2,i,j) + E(2,i+1,j)) 
    R(3) = dtdy * (-E(3,i,j) + E(3,i+1,j)) 
    R(4) = dtdy * (-E(4,i,j) + E(4,i+1,j)) 
    !R(5) = dtdydz * (-E(5,i,j,k) + E(5,i+1,j,k)) 
    ! y direction
    R(1) = R(1) + dtdx * (-F(1,i,j) + F(1,i,j+1))
    R(2) = R(2) + dtdx * (-F(2,i,j) + F(2,i,j+1))
    R(3) = R(3) + dtdx * (-F(3,i,j) + F(3,i,j+1))
    R(4) = R(4) + dtdx * (-F(4,i,j) + F(4,i,j+1))
  ...
  attributes(global) subroutine calc_step1(nx, ny, coef, dtdx, dtdy, E, F, Q, Q2)
    ...
    real(8), intent(in), device, contiguous  :: dtdx(nx-2,ny-2)   
    real(8), intent(in), device, contiguous  :: dtdy(nx-2,ny-2)   !<is this ok?
    !real(8), intent(in), device, contiguous  :: dtdzdx(nx-2,nz-2)   !< dt * Szx        <change E(5,-) to E(4,-)
    real(8), intent(in), device, contiguous  :: E(4,nx-1,ny-2) !< Flux in x direction
    real(8), intent(in), device, contiguous  :: F(4,nx-2,ny-1) !< Flux in y direction
    !real(8), intent(in), device, contiguous  :: G(5,nx-2,ny-2,nz-1) !< Flux in z direction
    real(8), intent(in), device, contiguous  :: Q(nx,4,ny)       !< present Q(rho, rhou, rhov, rhow, E) / Jacobian
    real(8), intent(out), device, contiguous :: Q2(nx,4,ny)      !< next    Q(rho, rhou, rhov, rhow, E) / Jacobian
    real(8) R(4), coef_dtdx, coef_dtdy !coef_dtdzdx
    integer i, j, l
    ...
    coef_dtdx = coef * dtdx(i,j)
    coef_dtdy = coef * dtdy(i,j)
    !coef_dtdzdx = coef * dtdzdx(i,k)
    call calc_R(nx, ny, i, j, coef_dtdx, coef_dtdy, E, F, R)
    do l = 1, 4  ! Loop over all conserved variables (rho, rhou, rhov, rhow, E)
      Q2(i+1,l,j+1) = Q(i+1,l,j+1) - R(l)
    enddo
  ...
  attributes(global) subroutine calc_step(nx, ny, coef1, coef2, dtdx, dtdy, E, F, Q, Q2, Rs)
    ...
    real(8), intent(in), device, contiguous    :: dtdx(nx-2,ny-2)    !< dt * Sxy
    real(8), intent(in), device, contiguous    :: dtdy(nx-2,ny-2)    !< dt * Syz
    ...
    do l = 1, 4
      ...
      Rs(i,l,j) = Rs(i,l,j) + coef2 * R(l)            ! Accumulate weighted residual
    enddo
  ...
  attributes(global) subroutine calc_step2_3(nx, ny, coef1, coef2, coef3, coef4_inv, dtdx, dtdy, E, F, Qin, Qout)
    ...
    coef3_dtdx = coef3 * dtdx(i,j)
    coef3_dtdy = coef3 * dtdy(i,j)
    !oef3_dtdzdx = coef3 * dtdzdx(i,k)
