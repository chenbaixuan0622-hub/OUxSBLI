...
contains 
  ...
  subroutine RungeKutta_3rd(id_RungeKutta, myrank, mygpu, nx, ny, x, dx_cpu, y, dy_cpu, Jacobian_cpu, Q)
    ...
    do t2 = 1, np
      if (mod(myrank,2) == 0) then
        ...
        do t1 = 1, nt
          ...
          call calc_EF(id_visc, nx, ny, xix, etay,  Jacobian, QJ, ruvwp, T, mu, E, F)
          ...
          call calc_EF(id_visc, nx, ny, xix, etay, Jacobian, QJ2, ruvwp, T, mu, E, F)
  ...
  subroutine RungeKutta_4th(id_RungeKutta, myrank, mygpu, nx, ny, x, dx_cpu, y, dy_cpu, Jacobian_cpu, Q)
    integer(4), intent(in) :: id_RungeKutta       !< time integration method ID
    integer, intent(in)    :: myrank              !< MPI rank
    integer, intent(in)    :: mygpu               !< GPU index for this rank
    integer, intent(in)    :: nx                  !< x grid dimension
    integer, intent(in)    :: ny                  !< y grid dimension
    ...
    do t2 = 1, np
      if (mod(myrank,2) == 0) then
        do t1 = 1, nt
          ...
          call calc_EF(id_visc, nx, ny, xix, etay, Jacobian, QJs, ruvwp, T, mu, E, F)
