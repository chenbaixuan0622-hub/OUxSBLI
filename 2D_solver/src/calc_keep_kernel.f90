...
contains
  ...
  attributes(global) subroutine calc_keep_x6(id_accuracy, nx, ny,  Q, T, E)
    ...
    real(8), dimension(-1:sx*sy-2), shared :: rho, u, v, p, tmp
  ...
  attributes(global) subroutine calc_keep_y6(id_accuracy, nx, ny, Q, T, F)
  ...
  attributes(global) subroutine calc_keep_x4(id_accuracy, nx, ny, Q, T, E)
    use mod_constant, only : Normal_x
    ...
    integer, parameter :: sy = threadsE%y     !< tile size in y direction
    real(8), dimension(0:sx*sy), shared :: rho, u, v, p, tmp
    integer(kind=2) id_accuracy2
    it = threadIdx%x
    jt = threadIdx%y
    j  = (blockIdx%y-1)*blockDim%y + jt + 1
  ...
  attributes(global) subroutine calc_keep_y4(id_accuracy, nx, ny, Q, T, F)
    use mod_constant, only : Normal_y
    integer(4), intent(in), value             :: id_accuracy    !< ID for accuracy, 4 means 4th-order
    integer, intent(in), value                :: nx             !< number of grid points in x direction
    integer, intent(in), value                :: ny             !< number of grid points in y direction
  ...
  attributes(global) subroutine calc_keep_x2(id_accuracy, nx, ny, Q, T, E)
    use mod_constant, only : Normal_x
    ...
    integer i,  j  !< global index in physical space
    integer it, jt !< local index in a block
    real(8), dimension(2) :: rho, u, v, p, tmp
    it = threadIdx%x
    jt = threadIdx%y
    i  = (blockIdx%x-1)*blockDim%x + it
    j  = (blockIdx%y-1)*blockDim%y + jt + 1
    if (nx-1 < i .or. ny-1 < j) return
    rho = Q(i:i+1,1,j); u = Q(i:i+1,2,j)
    v   = Q(i:i+1,3,j); p = Q(i:i+1,4,j); tmp = T(i:i+1,j)
    E(:,i,j-1) = KEEP(id_accuracy, rho, u, v, u, p, tmp, Normal_x)
  end subroutine calc_keep_x2


  !> CUDA Fortran kernel for 2nd-order KEEP scheme in y direction
  attributes(global) subroutine calc_keep_y2(id_accuracy, nx, ny, Q, T, F)
    use mod_constant, only : Normal_y
    integer(2), intent(in), value             :: id_accuracy    !< ID for accuracy, 2 means 2nd-order
    integer, intent(in), value                :: nx             !< number of grid points in x direction
    integer, intent(in), value                :: ny             !< number of grid points in y direction
    ...
    j  = (blockIdx%y-1)*blockDim%y + jt
    if (nx-1 < i .or. ny-1 < j ) return
    rho = Q(i,1,j:j+1); u = Q(i,2,j:j+1)
    v   = Q(i,3,j:j+1); p = Q(i,4,j:j+1); tmp = T(i,j:j+1)
    F(:,i-1,j) = KEEP(id_accuracy, rho, u, v, v, p, tmp, Normal_y)
