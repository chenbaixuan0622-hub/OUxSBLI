module calc_steps
  implicit none
contains
  attributes(global) subroutine calc_step1(nx,ny,nz,ni,nj,nk,dX,dY,dZ,dt,gamma,mu,kappa,Cp,Q,Q2,Q_d,R)
    use calc_flux
    integer, intent(in), value :: nx, ny, nz, ni, nj, nk
    real(8), intent(in), value :: dX, dY, dZ, dt, gamma, mu, kappa, Cp
    real(8), intent(in), dimension(nx,ny,nz,5,4) :: Q
    real(8), intent(inout), dimension(nx,ny,nz,5,4) :: Q2
    integer :: i, j, k
    ...
    Q2(i:i+ni-1,j:j+nj-1,k:k+nk-1,:,blockIdx%x) = Q(i:i+ni-1,j:j+nj-1,k:k+nk-1,:,blockIdx%x) - dt * R 
  end subroutine calc_step1
  
  attributes(global) subroutine calc_step2(nx,ny,nz,ni,nj,nk,dX,dY,dZ,dt,gamma,mu,kappa,Cp,Q,Q2,Q3,Q_d,R)
    use calc_flux
    integer, intent(in), value :: nx, ny, nz, ni, nj, nk
    real(8), intent(in), value :: dX, dY, dZ, dt, gamma, mu, kappa, Cp
    real(8), intent(in), dimension(nx,ny,nz,5,4) :: Q, Q2
    real(8), intent(inout), dimension(nx,ny,nz,5,4) :: Q3
    integer :: i, j, k
    real(8), intent(inout) :: Q_d(ni+4,nj+4,nk+4,5)
    real(8), intent(inout) :: R(ni,nj,nk,5)
    k = 3 + nk * mod(int(threadIdx%x/64),8) !k = 3, nz-nk-1, nk :3 <= k <= nz-2
  ...
  attributes(global) subroutine calc_step3(nx,ny,nz,ni,nj,nk,dX,dY,dZ,dt,gamma,mu,kappa,Cp,Q3,Q,Q_d,R)
    use calc_flux
    integer, intent(in), value :: nx, ny, nz, ni, nj, nk
    real(8), intent(in), value :: dX, dY, dZ, dt, gamma, mu, kappa, Cp
    real(8), intent(in), dimension(nx,ny,nz,5,4) :: Q3
    real(8), intent(inout), dimension(nx,ny,nz,5,4) :: Q
    integer :: i, j, k
    real(8), intent(inout) :: Q_d(ni+4,nj+4,nk+4,5)
    real(8), intent(inout) :: R(ni,nj,nk,5)
    k = 3 + nk * mod(int(threadIdx%x/64),8) !k = 3, nz-nk-1, nk :3 <= k <= nz-2
