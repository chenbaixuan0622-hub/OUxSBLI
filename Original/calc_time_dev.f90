...
contains
  ...
  subroutine RungeKutta(na,nx,ny,nz,nt,np,dx,dy,dz,dt,gamma,mu,kappa,Cp,Q)
    use calc_steps
    use calc_flux
    use set_bc
    use print
    integer, intent(in) :: na, nx, ny, nz, nt, np
    ...
    real(8) E(nx-na+1,ny-na,nz-na,5)
    real(8) F(nx-na,ny-na+1,nz-na,5)
    real(8) G(nx-na,ny-na,nz-na+1,5)
    dxi = 1.0d0 / dx
    dyi = 1.0d0 / dy
    dzi = 1.0d0 / dz
    ...
    do t2 = 1, nt
      do t1 = 1, np
        ...
        call calc_E(na,nx,ny,nz,gamma,rho,u,v,w,p,E)
        call calc_F(na,nx,ny,nz,gamma,rho,u,v,w,p,F)
        call calc_G(na,nx,ny,nz,gamma,rho,u,v,w,p,G)
        call calc_step1(na,nx,ny,nz,dxi,dyi,dzi,dt,E,F,G,Q,Q2)
        call set_cyclic_bc(na,nx,ny,nz,Q2)

        call calc_quantities(nx,ny,nz,gamma,Q2,rho,u,v,w,p)
        call calc_E(na,nx,ny,nz,gamma,rho,u,v,w,p,E)
        call calc_F(na,nx,ny,nz,gamma,rho,u,v,w,p,F)
        call calc_G(na,nx,ny,nz,gamma,rho,u,v,w,p,G)
        call calc_step2(na,nx,ny,nz,dxi,dyi,dzi,dt,E,F,G,Q,Q2,Q3)
        call set_cyclic_bc(na,nx,ny,nz,Q3)

        call calc_quantities(nx,ny,nz,gamma,Q3,rho,u,v,w,p)
        call calc_E(na,nx,ny,nz,gamma,rho,u,v,w,p,E)
        call calc_F(na,nx,ny,nz,gamma,rho,u,v,w,p,F)
        call calc_G(na,nx,ny,nz,gamma,rho,u,v,w,p,G)
        call calc_step3(na,nx,ny,nz,dxi,dyi,dzi,dt,E,F,G,Q3,Q)
