  ...
  pure attributes(device) function KEEP2(rho, u, p, T) result(F)
    ...
    F(3) = F(1) * 0.5d0 * (T(1) + T(2)) * R_over_gamma_1 ! internal energy
    F(3) = F(3) + 0.5d0 * (u(1) * p(2) + u(2) * p(1))     ! pressure diffusion
    F(3) = F(3) + 0.5d0 * F(1) * (u(1) * u(2))            ! kinetic energy
  end function KEEP2
