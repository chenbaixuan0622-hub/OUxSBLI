...
contains
  attributes(device) function KEP2(rho,p,u) result(F)
    ...
    ! KEP UP
    F(2) = 0.5d0 * (F(1) * (u(1) + u(2)) - abs(F(1)) * (u(2) - u(1)) + (p(1) + p(2)))

    ! original kinetic energy term
    !KE = 0.25d0 * F(1) * (u(1)**2 + u(2)**2)
    KE = 0.25d0 * (F(1) * (u(1)**2 + u(2)**2) - abs(F(1)) * (u(2)**2 - u(1)**2))
  ...
  attributes(device) function KEP4(rho,p,u,sensor) result(F)
    ...
    F(2) = 0.5d0 * (F(1) * (u(2) + u(3)) - sensor * abs(F(1)) * (u(3) - u(2)) + (p(2) + p(3)))
  ...
  attributes(device) function KEEP4(rho,p,u,sensor) result(F)
    real(8), intent(in), dimension(4), device :: rho, p, u
    real(8), intent(in), value                :: sensor
    real(8), dimension(3) :: F
    real(8) KE, IE, PV
