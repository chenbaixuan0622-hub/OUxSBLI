...
contains
  ...
  function al(a,k,b,d_p,d_m) result(ans)
    real(8), intent(in) :: a, k, b, d_p, d_m
  ...
  function ar(a,k,b,d_p,d_m) result(ans)
    real(8), intent(in) :: a, k, b, d_p, d_m
    real(8) :: ans, delta_p, delta_m
    delta_p = minmod(d_p, b * d_m)
    delta_m = minmod(d_m, b * d_p)
  ...
  subroutine MUSCL(dim,k,b,Q1,Q2,d1,d2,d3,Ql,Qr)
    integer, intent(in), value :: dim
    real(8), intent(in) :: k, b
    real(8), intent(in), dimension(dim) :: Q1, Q2, d1, d2, d3
    real(8), intent(out), dimension(dim) :: Ql, Qr
    integer i
    do i = 1, dim
