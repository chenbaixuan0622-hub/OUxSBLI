module set
  ...
  & beta, theta, M0, a1, rho0, rho2, p0, p2, u1, u2, v1, v2, u_magnitude, ux, uy
  implicit none
contains
  ...
  subroutine set_grid(nx,ny,nz,x,y,z,dx,dy)
    ...
    y(1) = 0.d0
    do j = 1, 256
      dy(j) = max(0.025d0, 1.25d0 * dble(j)/dble(256)) * dy1
      y(j+1) = y(j) + dy(j)
    enddo
    do j = 257, ny-1
      dy(j) = 1.25d0 * dy1 
  ...
  subroutine set_bc(Jacobian,QJ)
    real(8), intent(in), device         :: Jacobian(nx,ny)
    real(8), intent(inout), device      :: QJ(nx,ny,nz,5)
    integer i, j, k, l
    integer :: No = int(0.35 * nx)
    integer :: Nre = int(0.3 * nx)
    real(8) :: p_wall
    ! Riemann invariants
