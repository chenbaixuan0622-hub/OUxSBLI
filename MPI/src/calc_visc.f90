module calc_visc
  use mod_globals, only : id_visc, id_turbulence, gamma, R, Pr, Prt, dzi
  use calc_Sutherland
  implicit none
contains
  ...
  attributes(global) subroutine calc_Ev(nx, ny, nz, dx, dy, rho, u, v, w, T, p, mut, E)
    ...
    real(8), intent(inout), device                    :: E(nx-1,ny-2,nz-2,5)
    integer i, j, k
    real(8) :: mux = 0.d0
    real(8) :: muy1 = 0.d0
    real(8) :: muy2 = 0.d0
    ...
    E(i-1+1,j-1,k-1,2) = E(i-1+1,j-1,k-1,2) - (txx+txxsgs)
    E(i-1+1,j-1,k-1,3) = E(i-1+1,j-1,k-1,3) - (txy+txysgs)
    E(i-1+1,j-1,k-1,4) = E(i-1+1,j-1,k-1,4) - (txz+txzsgs)
    E(i-1+1,j-1,k-1,5) = E(i-1+1,j-1,k-1,5) &
  ...
  attributes(global) subroutine calc_Fv(nx, ny, nz, dy, dx, rho, u, v, w, T, p, mut, F)
    ...
    real(8), intent(in), dimension(nx-1), device      :: dx ! 1 / dx
    real(8), intent(in), dimension(nx,ny,nz), device  :: rho, u, v, w, T, p, mut
    real(8), intent(inout), device                    :: F(nx-2,ny-1,nz-2,5)
    integer i, j, k
    real(8) :: muy = 0.d0
    real(8) :: muz1 = 0.d0
    real(8) :: muz2 = 0.d0
    ...
    F(i-1,j-1+1,k-1,2) = F(i-1,j-1+1,k-1,2) - (tyx+tyxsgs)
    F(i-1,j-1+1,k-1,3) = F(i-1,j-1+1,k-1,3) - (tyy+tyysgs)
    F(i-1,j-1+1,k-1,4) = F(i-1,j-1+1,k-1,4) - (tyz+tyzsgs)
    F(i-1,j-1+1,k-1,5) = F(i-1,j-1+1,k-1,5) &
  ...
  attributes(global) subroutine calc_Gv(nx, ny, nz, dx, dy, rho, u, v, w, T, p, mut, G)
    ...
    real(8), intent(in), dimension(ny-1), device      :: dy ! 1 / dy
    real(8), intent(in), dimension(nx,ny,nz), device  :: rho, u, v, w, T, p, mut
    real(8), intent(inout), device                    :: G(nx-2,ny-2,nz-1,5)
    integer i, j, k
    real(8) :: muz = 0.d0
    real(8) :: mux1 = 0.d0
    real(8) :: mux2 = 0.d0
    ...
    G(i-1,j-1,k-1+1,2) = G(i-1,j-1,k-1+1,2) - (tzx+tzxsgs)
    G(i-1,j-1,k-1+1,3) = G(i-1,j-1,k-1+1,3) - (tzy+tzysgs)
    G(i-1,j-1,k-1+1,4) = G(i-1,j-1,k-1+1,4) - (tzz+tzzsgs)
    G(i-1,j-1,k-1+1,5) = G(i-1,j-1,k-1+1,5) &
