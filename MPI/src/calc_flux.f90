module calc_flux
  use mod_globals, only : id_scheme, gamma
  use calc_qlr
  use calc_keep
  use calc_slau
  use calc_roe
...
!MUSCL!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  attributes(global) subroutine calc_E_MUSCL(id_muscl, nx, ny, nz, rho, u, v, w, p, E)
    ...
    if (id_scheme == 2) then
      E(i,j-1,k-1,:) = Roe(1,Ql,Qr,Normal)
    else
      E(i,j-1,k-1,:) = SLAU(1,Ql,Qr,Normal)
    endif
  ...
  attributes(global) subroutine calc_F_MUSCL(id_muscl, nx, ny, nz, rho, u, v, w, p, F)
    ...
    if (id_scheme == 2) then
      F(i-1,j,k-1,:) = Roe(2,Ql,Qr,Normal)
    else
      F(i-1,j,k-1,:) = SLAU(2,Ql,Qr,Normal)
    endif
  ...
  attributes(global) subroutine calc_G_MUSCL(id_muscl, nx, ny, nz, rho, u, v, w, p, G)
    ...
    if (id_scheme == 2) then
      G(i-1,j-1,k,:) = Roe(3,Ql,Qr,Normal)
    else
      G(i-1,j-1,k,:) = SLAU(3,Ql,Qr,Normal)
    endif
