...
contains
  ...
  attributes(device) function MUSCL3rdnonTVD(id_tvd,eps1,eps2,k,a2,a3,d) result(alr)
    integer(kind=2), intent(in), value :: id_tvd
    real(8), intent(in), value         :: eps1, eps2, k, a2, a3
    real(8), intent(in), device        :: d(3)
    real(8) :: b, alr(2)
    b      = (3.d0 - k) / (1.d0 -k)
    alr(1) = a2 + 0.25d0 * eps1 * ((1.d0 - k) * d(1) + (1.d0 + K) * d(2))
    alr(2) = a3 - 0.25d0 * eps2 * ((1.d0 - k) * d(3) + (1.d0 + K) * d(2))
  end function MUSCL3rdnonTVD

  attributes(device) function MUSCL3rdMinmod(id_tvd,eps1,eps2,k,a2,a3,d) result(alr)
    ...
    alr(1) = a2 + 0.25d0 * eps1 * ((1.d0 - k) * dt1 + (1.d0 + K) * dt2)
    alr(2) = a3 - 0.25d0 * eps2 * ((1.d0 - k) * dt3 + (1.d0 + K) * dt4)
  end function MUSCL3rdMinmod

  attributes(device) function MUSCL3rdpost(id_tvd,eps1,eps2,k,a2,a3,d) result(alr)
    ...
    integer(kind=4) :: id4
    integer i
    au  = MUSCL3rdnonTVD(id2,eps1,eps2,k,a2,a3,d)
    al  = MUSCL3rdMinmod(id4,eps1,eps2,k,a2,a3,d)
    do i = 1, 2
  ...
  attributes(device) function delta4(eps1,eps2,k,a) result(alr)
    use mod_globals, only : id_tvd
    real(8), intent(in), value  :: eps1, eps2, k
    real(8), intent(in), device :: a(4)
    real(8) :: alr(2), d(3)
    d(:) = -a(1:3) + a(2:4)
    alr  = MUSCL3rd(id_tvd,eps1,eps2,k,a(2),a(3),d)
  end function delta4

  attributes(device) function delta6(a) result(alr)
    use mod_globals, only : id_tvd
  ...
  attributes(device) subroutine calc_4points(eps1,eps2,k,rho,p,V,rho2,p2,V2)
    real(8), intent(in), value   :: eps1, eps2, k
    real(8), intent(in), device  :: rho(4),  p(4),  V(4,3)
    real(8), intent(out), device :: rho2(2), p2(2), V2(2,3)
    real(8) Vtemp(4)
    rho2(:) = delta4(eps1,eps2,k,rho)
    p2(:)   = delta4(eps1,eps2,k,p)
    Vtemp   = V(:,1)
    V2(:,1) = delta4(eps1,eps2,k,Vtemp)
    Vtemp   = V(:,2)
    V2(:,2) = delta4(eps1,eps2,k,Vtemp)
    Vtemp   = V(:,3)
    V2(:,3) = delta4(eps1,eps2,k,Vtemp)
  end subroutine calc_4points

  attributes(device) subroutine calc_6points(rho,p,V,rho2,p2,V2)
