...
contains
  ...
  attributes(device) function al(a,eps,k,b,d_p,d_m) result(ans)
  ...
  attributes(device) function ar(a,eps,k,b,d_p,d_m) result(ans)
    real(8), intent(in), value :: a, eps, k, b, d_p, d_m
    real(8) :: ans, delta_p, delta_m
    delta_p = minmod(d_p, b * d_m)
    delta_m = minmod(d_m, b * d_p)
  ...
  attributes(device) subroutine MUSCL(dim,eps,k,b,Q1,Q2,d1,d2,d3,Ql,Qr)
    integer, intent(in), value :: dim
    real(8), intent(in), value :: eps, k, b
    real(8), intent(in), dimension(dim), device :: Q1, Q2, d1, d2, d3
    real(8), intent(out), dimension(dim), device :: Ql, Qr
    integer i
    do i = 1, dim
      Ql(i) = al(Q1(i), eps, k, b, d2(i), d1(i))
      Qr(i) = ar(Q2(i), eps, k, b, d3(i), d2(i))
    enddo
