module mod_globals
  ...
  ! peak wavenumber for energy spectrum E(k) = k^4 exp(-2*(k/kp)^2)
  real(8), parameter :: kp   = 4.d0

  ! dt is acoustic-limited: c_s = V0/M0 dominates advective speed V0
  real(8), parameter :: CFL = 0.03d0
  real(8), parameter :: dt  = CFL * (Lx / dble(nx-1)) * M0 / V0
  real(8), parameter :: dtn = V0 * dt / L0
  integer, parameter :: np  = 50
  integer, parameter :: nt  = int(10.d0 / (dble(np) * dtn))
