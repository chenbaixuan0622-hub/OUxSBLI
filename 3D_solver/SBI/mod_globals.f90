module mod_globals
  ...
  ! mesh
  real(8), parameter :: Lx = 40d-3
  real(8), parameter :: Ly = 8d-3
  real(8), parameter :: Lz = 0.5d-3
  integer, parameter :: nx = 1025
  integer, parameter :: ny = 257
  integer, parameter :: nz = 33

  integer, parameter :: nre1 = int(0.35 * nx)
  integer, parameter :: nre2 = int(0.4 * nx)

  ! RTX 4090
  type(dim3) :: blocksE   = dim3((nx-accuracy+1)/64,(ny-accuracy)/5,(nz-accuracy)/1)
  type(dim3) :: blocksF   = dim3((nx-accuracy)/3,(ny-accuracy+1)/64,(nz-accuracy)/1)
  type(dim3) :: blocksG   = dim3((nx-accuracy)/3,(ny-accuracy)/1,(nz-accuracy+1)/32)
  type(dim3) :: blocksEv  = dim3((nx-accuracy+1)/32,(ny-accuracy)/5,(nz-accuracy)/1)
  type(dim3) :: blocksFv  = dim3((nx-accuracy)/3,(ny-accuracy+1)/32,(nz-accuracy)/1)
  type(dim3) :: blocksGv  = dim3((nx-accuracy)/3,(ny-accuracy)/1,(nz-accuracy+1)/32)
  type(dim3) :: blocks    = dim3((nx-accuracy)/31,(ny-accuracy)/5,(nz-accuracy)/1)
  type(dim3) :: threadsE  = dim3(64,5,1)
  type(dim3) :: threadsF  = dim3(3,64,1)
  type(dim3) :: threadsG  = dim3(3,1,64)
  type(dim3) :: threadsEv = dim3(32,5,1)
  type(dim3) :: threadsFv = dim3(3,32,1)
  type(dim3) :: threadsGv = dim3(3,1,32)
  type(dim3) :: threads   = dim3(31,5,1)
