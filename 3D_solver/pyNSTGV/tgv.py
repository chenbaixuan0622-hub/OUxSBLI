...
def set_init(nx, ny, nz, x, y, z, gamma, Rgas, rho0, v0, p0, T0, L0):
  Q = np.zeros((nz,ny,nx,5), dtype=np.float64)
  for k in range(nz):
    for j in range(ny):
      for i in range(nx):
        p   = p0 + rho0 * (v0**2) * (np.cos(2.e0 * x[i]/L0)+np.cos(2.e0 * y[j] / L0)) \
            * (np.cos(2.e0 * z[k] / L0) + 2.e0) / 16.e0
        rho = p / (Rgas * T0)
        Q[k,j,i,0] =  rho
        Q[k,j,i,1] =  rho * v0 * np.sin(x[i]/L0) * np.cos(y[j]/L0) * np.cos(z[k]/L0)
        Q[k,j,i,2] = -rho * v0 * np.cos(x[i]/L0) * np.sin(y[j]/L0) * np.cos(z[k]/L0)
        Q[k,j,i,3] = 0.e0
