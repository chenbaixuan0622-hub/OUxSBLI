...
contains
  ...
  subroutine GPU_SENDRECV(myrank,nx,ny,nz,QJ,Qd)
    integer, intent(in)          :: myrank, nx, ny, nz
    real(8), intent(in), device  :: QJ(nx,ny,nz,5)
    real(8), intent(out), device :: Qd(nx,2,nz,5)
    real(8), allocatable, pinned :: Qp1(:,:,:,:), Qp2(:,:,:,:)
    integer ierr, status(MPI_STATUS_SIZE)
    allocate(Qp1(nx,2,nz,5),Qp2(nx,2,nz,5))
    if (myrank == 0) then
      Qp1 = QJ(:,3:4,:,:)
      call MPI_SENDRECV(Qp1, 10*nx*nz, MPI_REAL8, 2, 0, &
                        Qp2, 10*nx*nz, MPI_REAL8, 2, 0, MPI_COMM_WORLD, status, ierr)
      Qd = Qp2
    else
      Qp1 = QJ(:,ny-3:ny-2,:,:)
      call MPI_SENDRECV(Qp1, 10*nx*nz, MPI_REAL8, 0, 0, &
                        Qp2, 10*nx*nz, MPI_REAL8, 0, 0, MPI_COMM_WORLD, status, ierr)
      Qd = Qp2
    endif
    deallocate(Qp1,Qp2)
  end subroutine
  ...
  subroutine set_bc1(nx,ny,nz,Jacobian,QJ)
    ...
    ! parallel
    call GPU_SENDRECV(0,nx,ny,nz,QJ,Qd)
    !$cuf kernel do(3)<<<*,*>>>
    do l = 1, 5
      do k = 1, nz
        do i = 1, nx
          QJ(i,1,k,l) = Qd(i,1,k,l)
          QJ(i,2,k,l) = Qd(i,2,k,l)
    enddo;enddo;enddo
  ...
  subroutine set_bc2(nx,ny,nz,Jacobian,QJ)
    ...
    ! parallel
    call GPU_SENDRECV(2,nx,ny,nz,QJ,Qd)
    !$cuf kernel do(3)<<<*,*>>>
    do l = 1, 5
      do k = 1, nz
        do i = 1, nx
          QJ(i,ny-1,k,l) = Qd(i,1,k,l)
          QJ(i,ny,k,l)   = Qd(i,2,k,l)
    enddo;enddo;enddo
