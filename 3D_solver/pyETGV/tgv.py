...
def set_init(nx, ny, nz, x, y, z, gamma, Rgas, M0, rho0):
  Q = np.zeros((5, nz, ny, nx), dtype=np.float64)
  for k in range(nz):
    for j in range(ny):
      for i in range(nx):
        Q[0,k,j,i] =  rho0
        Q[1,k,j,i] =  rho0 * M0 * np.sin(x[i]) * np.cos(y[j]) * np.cos(z[k])
        Q[2,k,j,i] = -rho0 * M0 * np.cos(x[i]) * np.sin(y[j]) * np.cos(z[k])
        Q[3,k,j,i] = 0.e0
        Q[4,k,j,i] = (1.e0 / gamma + 0.0625e0 * rho0 * (M0**2) * \
        (np.cos(2.e0 * x[i]) + np.cos(2.e0 * y[j])) * (np.cos(2.e0 * z[k]) + 2.e0) \
        ) / (gamma - 1.e0) + 0.5e0 * (Q[1,k,j,i]**2 + Q[2,k,j,i]**2) / Q[0,k,j,i]
  # x direction
