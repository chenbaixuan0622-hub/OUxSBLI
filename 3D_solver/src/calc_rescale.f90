...
contains
  subroutine calc_mean(step, flag_re, nx, ny, nz, Jacobian, QJ, Qm)
    ...
    real(8), intent(inout), device :: Qm(ny*4)
    real(8) Q1, Q2, Q3, Q4, rhoinv, Jacobian_tmp, volinv, step1, step2
    integer i, k, istat
      ...
      do j = 1, ny
        ...
        do k = 4, nz-3
          do i = nre1, nre2
            ...
            Q4 = Q4 + gamma_1 * Jacobian_tmp * (QJ(5,i,j,k) &
                    - 0.5d0 * (QJ(2,i,j,k)**2 + QJ(3,i,j,k)**2 + QJ(4,i,j,k)**2) * rhoinv)
        enddo;enddo
        Qm(4*(j-1)+1) = Q1 * volinv
        Qm(4*(j-1)+2) = Q2 * volinv
        Qm(4*(j-1)+3) = Q3 * volinv
        Qm(4*(j-1)+4) = Q4 * volinv
      enddo
    else
        ...
        do k = 4, nz-3
          do i = nre1, nre2
            ...
            Q4 = Q4 + gamma_1 * Jacobian_tmp * (QJ(5,i,j,k) &
                    - 0.5d0 * (QJ(2,i,j,k)**2 + QJ(3,i,j,k)**2 + QJ(4,i,j,k)**2) * rhoinv)
        enddo;enddo
        Qm(4*(j-1)+1) = (step1 * Qm(4*(j-1)+1) + Q1 * volinv) * step2
        Qm(4*(j-1)+2) = (step1 * Qm(4*(j-1)+2) + Q2 * volinv) * step2
        Qm(4*(j-1)+3) = (step1 * Qm(4*(j-1)+3) + Q3 * volinv) * step2
        Qm(4*(j-1)+4) = (step1 * Qm(4*(j-1)+4) + Q4 * volinv) * step2
      enddo
    endif
  ...
  subroutine step_rescale(num, myrank, step, nx, ny, nz, flag_re, ireq, ireq2, Jacobian, QJ, Qm, Qre)
    ...
    real(8), intent(in), device    :: Jacobian(nx,ny), QJ(5,nx,ny,nz)
    real(8), intent(inout), device :: Qm(ny*4), Qre(ny*(nz-6)*5)
    ...
    if (myrank == rerank) then
      ...
      if (num == 1 .and. id_recal == 0) then
        ...
        call CPUGPU_MPI_SEND(id_gpumpi, Qm, 4*ny, rerank+1, 1, ireq2(2), ierr)
      endif
    endif
    if (myrank == 0) then
  ...
  subroutine rescale_recv_send(num, flag_re, nx, ny, nz, step, y, Jacobian, Qm_cpu)
    ...
    real(8), intent(inout) :: Qm_cpu(ny*4)
    real(8)         :: Qre_cpu(ny*(nz-6)*5), bltre
    real(8), device ::     Qre(ny*(nz-6)*5), Qm(ny*4)
    ...
    if (num == 1 .and. id_recal == 0) then
      ...
      call CPUGPU_MPI_RECV(id_gpumpi, Qm,  4*ny,        rerank, 1, ireqs(2), ierr)
      call MPI_WAITALL(2, ireqs, istats, ierr)
      stat = cudaDeviceSynchronize()
      stat = cudaMemcpy(Qm_cpu, Qm, 4*ny, cudaMemcpyDeviceToHost)
    ...
    stat = cudaDeviceSynchronize()
    call set_rescale_Sagaut(flag_re, step, nx, ny, nz-6, y, Jacobian, Qm_cpu, bltre, Qre_cpu)
  ...
  subroutine write_Qm(ny, y, Qm)
    ...
    do j = 1, ny
      jj = 4 * (j-1)
      rho = Qm(jj+1); u = Qm(jj+2); v = Qm(jj+3); p = Qm(jj+4)
      write(10, "(6e12.4)") y(j), rho, u, v, p
    enddo
    close(10)
  ...
  subroutine multiply_Jacobian(nx, ny, nz, Jacobian, Qre)
    integer, intent(in)    :: nx, ny, nz
    ...
    enddo;enddo;enddo
  end subroutine multiply_Jacobian


  subroutine set_cyclic_spanwise(ny, nz, Jacobian, Qre)
    integer, intent(in)    :: ny, nz
    real(8), intent(in)    :: Jacobian(nx,ny)
    real(8), intent(inout) :: Qre(ny*nz*5)
    integer j, k, l, j_offset, k_offset
    do k = 1, nz
      k_offset = ny * 5 * (k-1)
      do j = 1, ny
    ...
    enddo;enddo;enddo
  end subroutine set_cyclic_spanwise


  function blt99(ny, u0, y, Um) result(bltre)
    integer, intent(in) :: ny
    real(8), intent(in) :: u0, y(ny), Um(ny)
    real(8) dudy, bltre, bltup, bltdown
    real(8) :: blt_min = 0.5d0 * blt, blt_max = 2.d0 * blt
    integer j, jup, jdown, flag, ierr, errorcode
    flag  = 0
    do j = 2, ny
      if (Um(j) >= 0.99e0 * u0) then
    ...
    enddo
    !if (jup /= jdown) then
    !  print *, "blt up and blt down are different", bltup, bltdown
    !endif
  ...
  subroutine calc_wall_unit(ny, rhow, Tw, y, Um, nu, utre, utin, beta)
    integer, intent(in)  :: ny
    real(8), intent(in)  :: rhow, Tw, y(ny), Um(ny)
    real(8), intent(out) :: nu, utre, utin, beta
    real(8) mu, over_rhow
    over_rhow = 1.d0 / rhow
    ...
    beta  = (bltre / blt)**0.1
    utin  = beta * utre
  end subroutine calc_wall_unit


  subroutine inner_outer_scale(ny, utin, utre, nu, bltre, y, ypin, ypre, etin, etre, weight)
    integer, intent(in)  :: ny
    real(8), intent(in)  :: utin, utre, nu, bltre, y(ny)
    real(8), intent(out) :: ypin(ny), ypre(ny), etin(ny), etre(ny), weight(ny)
    real(8) utin_nu, utre_nu, over_blt, over_bltre
    integer j
    utin_nu = utin / nu
    utre_nu = utre / nu
    over_blt   = 1.d0 / blt
    ...
    enddo
  end subroutine inner_outer_scale


  subroutine set_rescale_Sagaut(flag_re, step, nx, ny, nz, y, Jacobian, Qm, bltre, Qre)
    integer, intent(inout) :: flag_re
    integer, intent(in)    :: step, nx, ny, nz! nz-6
    real(8), intent(in)    :: y(ny), Jacobian(nx,ny), Qm(ny*4)
    real(8), intent(out)   :: bltre
    real(8), intent(inout) :: Qre(ny*nz*5) ! Q / J
    integer i, j, jj, k, kh, l, j_offset, k_offset
    integer, dimension(ny) :: jj_y, jj_e
    real(8) t, utre, utin, beta, nu, ady, ade, one_ady, one_ade 
    ! mean properties at rescaling plane
    real(8), dimension(ny)    :: Um, Vm, rhom, Tm, pm
    ...
    call multiply_Jacobian(nx, ny, nz, Jacobian, Qre)
    ...
    if (flag_re >= 1 .and. step >= start_rescale) then
      ...
      call calc_wall_unit(ny, rhom(1), Tm(1), y, Um, nu, utre, utin, beta)
      call inner_outer_scale(ny, utin, utre, nu, bltre, y, ypin, ypre, etin, etre, weight)
      ...
      call set_cyclic_spanwise(ny, nz, Jacobian, Qre)
    endif
  end subroutine set_rescale_Sagaut


  subroutine set_rescale_Pirozzoli(flag_re, step, nx, ny, nz, y, Jacobian, Qm, bltre, Qre)
    integer, intent(inout) :: flag_re
    integer, intent(in)    :: step, nx, ny, nz! nz-6
    real(8), intent(in)    :: y(ny), Jacobian(nx,ny), Qm(ny*4)
    real(8), intent(out)   :: bltre
    ...
    real(8) t, utre, utin, beta, nu, ady, ade, one_ady, one_ade 
    ! mean properties at rescaling plane
    real(8), dimension(ny)    :: Um, Vm, rhom
    ! fluctuating properties at rescaling plane
    real(8), dimension(ny,nz) :: ufre, vfre, wfre, rhofre
    real(8), dimension(ny)    :: ypre, ypin, etre, etin
    ! fluctuating properties at both inner and outer region
    real(8), dimension(ny,nz) :: ufin, vfin, wfin, rhofin
    real(8), dimension(ny,nz) :: ufout, vfout, wfout, rhofout
    ! mean properties at both inner and outer region
    real(8), dimension(ny)    :: Umin, Vmin, rhomin, Umout, Vmout, rhomout
    ! weighting function
    real(8), dimension(ny)    :: weight
    ! properties at rescaling plane
    real(8) ure, vre, wre, rhore, pre
    ! rescaled properties at inlet
    real(8) uin, vin, win, rhoin
    ! cache
    real(8) :: u_tmp, v_tmp, p_tmp, rho_tmp, T_tmp, weight_tmp, Jacobian_tmp, over_rhore
    call multiply_Jacobian(nx, ny, nz, Jacobian, Qre)
    
    do j = 1, ny
      u_tmp      = Qm(4*(j-1)+2)
      v_tmp      = Qm(4*(j-1)+3)
      T_tmp      = Tw - rf * u_tmp**2 / (2.0d0 * Cp) ! Crocco-Busemann
      rho_tmp    = p0 / (R * T_tmp)
      rhom(j)    = rho_tmp
      Um(j)      = u_tmp
      Umin(j)    = u_tmp
    ...
    if (flag_re >= 1 .and. step >= start_rescale) then
      ufin(:,:)  = 0.d0; vfin(:,:)  = 0.d0; wfin(:,:)  = 0.d0; rhofin(:,:)  = 0.d0
      ufout(:,:) = 0.d0; vfout(:,:) = 0.d0; wfout(:,:) = 0.d0; rhofout(:,:) = 0.d0
      do k = 1, nz
        k_offset = ny * 5 * (k-1)
        do j = 1, ny
          j_offset = 5 * (j-1)
      ...
      call calc_wall_unit(ny, rhom(1), p0 / (rhom(1)*R), y, Um, nu, utre, utin, beta)
      call inner_outer_scale(ny, utin, utre, nu, bltre, y, ypin, ypre, etin, etre, weight)
      ...
      do j = 1, ny
        do jj = 2, ny
          if (ypre(jj) > ypin(j)) then
            ...
            Umin(j)   = beta * (one_ady *   Um(jj-1) + ady *   Um(jj))!  Um(jj-1) + ady * (-  Um(jj-1) +   Um(jj))
            Vmin(j)   =         one_ady *   Vm(jj-1) + ady *   Vm(jj) !  Vm(jj-1) + ady * (-  Vm(jj-1) +   Vm(jj))
            rhomin(j) =         one_ady * rhom(jj-1) + ady * rhom(jj) !rhom(jj-1) + ady * (-rhom(jj-1) + rhom(jj))
            jj_y(j)   = jj
            exit
          endif
      enddo;enddo
      ...
      do j = 1, ny
        do jj = 2, ny
          if (etre(jj) > etin(j)) then
            ...
            Umout(j)   = beta * (one_ade *   Um(jj-1) + ade *   Um(jj)) + (1.d0 - beta) * u0
            Vmout(j)   =         one_ade *   Vm(jj-1) + ade *   Vm(jj) !  Vm(jj-1) + ade * (-  Vm(jj-1) +   Vm(jj))
            rhomout(j) =         one_ade * rhom(jj-1) + ade * rhom(jj) !rhom(jj-1) + ade * (-rhom(jj-1) + rhom(jj))
            jj_e(j)    = jj
            exit
          endif
      enddo;enddo
      ...
      do k = 1, nz
        do j = 1, ny
          ...
          if (jj > 0) then
            ...
            vfin(j,k)   = beta * (one_ady *   vfre(jj-1,k) + ady *   vfre(jj,k))!  vfre(jj-1,k) + ady * (-  vfre(jj-1,k) +   vfre(jj,k))
            wfin(j,k)   = beta * (one_ady *   wfre(jj-1,k) + ady *   wfre(jj,k))!  wfre(jj-1,k) + ady * (-  wfre(jj-1,k) +   wfre(jj,k))
            rhofin(j,k) =         one_ady * rhofre(jj-1,k) + ady * rhofre(jj,k) !rhofre(jj-1,k) + ady * (-rhofre(jj-1,k) + rhofre(jj,k))
          endif
          jj = jj_e(j)
          if (jj > 0) then
            ade     = (-etre(jj-1) + etin(j)) / (-etre(jj-1) + etre(jj))
            ...
            vfout(j,k)   = beta * (one_ade *   vfre(jj-1,k) + ade *   vfre(jj,k))!  vfre(jj-1,k) + ade * (-  vfre(jj-1,k) +   vfre(jj,k))
            wfout(j,k)   = beta * (one_ade *   wfre(jj-1,k) + ade *   wfre(jj,k))!  wfre(jj-1,k) + ade * (-  wfre(jj-1,k) +   wfre(jj,k))
            rhofout(j,k) =         one_ade * rhofre(jj-1,k) + ade * rhofre(jj,k) !rhofre(jj-1,k) + ade * (-rhofre(jj-1,k) + rhofre(jj,k))
          endif
      enddo;enddo
      ...
      do k = 1, nz
        ...
        do j = 1, ny
          ...
          vin   = (  Vmin(j) +   vfin(j,kh)) * (1.d0 - weight_tmp) + (  Vmout(j) +   vfout(j,kh)) * weight_tmp
          win   =                wfin(j,kh)  * (1.d0 - weight_tmp) +                 wfout(j,kh)  * weight_tmp
          rhoin = (rhomin(j) + rhofin(j,kh)) * (1.d0 - weight_tmp) + (rhomout(j) + rhofout(j,kh)) * weight_tmp
          Qre(k_offset+j_offset+1) = rhoin * Jacobian_tmp
          Qre(k_offset+j_offset+2) = rhoin * uin * Jacobian_tmp
          Qre(k_offset+j_offset+3) = rhoin * vin * Jacobian_tmp
          Qre(k_offset+j_offset+4) = rhoin * win * Jacobian_tmp
          Qre(k_offset+j_offset+5) = (p0 * over_gamma_1 + 0.5d0 * rhoin * (uin**2 + vin**2 + win**2)) * Jacobian_tmp
      enddo;enddo
    else
      call set_cyclic_spanwise(ny, nz, Jacobian, Qre)
    endif
  end subroutine set_rescale_Pirozzoli
