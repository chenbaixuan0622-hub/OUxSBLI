...
contains
  ...
  !> Compute div(u) at cell-center (4th-order, interior stencil).
  !! ux/vy/wz are gated independently since each only depends on its own
  !! direction's neighbours -- a point can be in-range for one component
  !! (e.g. vy) while too close to the array edge for another (e.g. ux).
  subroutine calc_div_4_in(nx, ny, nz, xix, etay, zetaz, Q_2, Q_3, Q_4, ux, vy, wz)
    integer, intent(in), value                        :: nx, ny, nz
    real(8), intent(in), dimension(nx-1), device      :: xix   ! 1 / dx
    real(8), intent(in), dimension(ny-1), device      :: etay  ! 1 / dy
    real(8), intent(in), dimension(nz-1), device      :: zetaz ! 1 / dz
    ...
    do k = 1, nz
      ...
      enddo
    enddo
  end subroutine calc_div_4_in

  !> Compute div(u) at cell-center (6th-order only, interior stencil).
  !! See calc_div_4_in for why ux/vy/wz are gated independently.
  subroutine calc_div_6_in(nx, ny, nz, xix, etay, zetaz, Q_2, Q_3, Q_4, ux, vy, wz)
    integer, intent(in), value                        :: nx, ny, nz
    real(8), intent(in), dimension(nx-1), device      :: xix   ! 1 / dx
    real(8), intent(in), dimension(ny-1), device      :: etay  ! 1 / dy
    real(8), intent(in), dimension(nz-1), device      :: zetaz ! 1 / dz
