  ...
  attributes(global) subroutine calc_slau_x6(id_accuracy, nx, ny, nz, Q, sensor, E)
    ...
    block
      ...
      if (3 <= i .and. i <= nx-3) then
        ...
        wiggle = wiggle_detector(p(it-1:it+2,jt,kt))
    ...
    associate(un1 => u(it,jt,kt), un2 => ur(it,jt,kt))
      call SLAU(id_slau, rho(it,jt,kt), rhor(it,jt,kt), u(it,jt,kt), ur(it,jt,kt), v(it,jt,kt), vr(it,jt,kt), &
                w(it,jt,kt), wr(it,jt,kt), un1, un2, p(it,jt,kt), pr(it,jt,kt), Normal_x, wiggle, E(:,i,j-1,k-1))
    end associate
  ...
  attributes(global) subroutine calc_slau_y6(id_accuracy, nx, ny, nz, Q, sensor, F)
    ...
    block
      ...
      if (3 <= j .and. j <= ny-3 .and. 8 <= kind(id_accuracy)) then
        ...
        wiggle = wiggle_detector(p(jt-1:jt+2,it,kt))
    ...
    associate(un1 => v(jt,it,kt), un2 => vr(jt,it,kt))
      call SLAU(id_slau, rho(jt,it,kt), rhor(jt,it,kt), u(jt,it,kt), ur(jt,it,kt), v(jt,it,kt), vr(jt,it,kt), &
                w(jt,it,kt), wr(jt,it,kt), un1, un2, p(jt,it,kt), pr(jt,it,kt), Normal_y, wiggle, F(:,i-1,j,k-1))
    end associate
  ...
  attributes(global) subroutine calc_slau_z6(id_accuracy, nx, ny, nz, Q, sensor, G)
    ...
    block
      ...
      if (3 <= k .and. k <= nz-3 .and. 8 <= kind(id_accuracy)) then
        ...
        wiggle = wiggle_detector(p(k-1:kt+2,jt,it))
    ...
    associate(un1 => w(kt,jt,it), un2 => wr(kt,jt,it))
      call SLAU(id_slau, rho(kt,jt,it), rhor(kt,jt,it), u(kt,jt,it), ur(kt,jt,it), v(kt,jt,it), vr(kt,jt,it), &
                w(kt,jt,it), wr(kt,jt,it), un1, un2, p(kt,jt,it), pr(kt,jt,it), Normal_z, wiggle, G(:,i-1,j-1,k))
    end associate
