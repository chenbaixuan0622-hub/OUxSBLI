...
contains
  subroutine RungeKutta(T0,Q)
    use cudafor
    use mod_globals, only : accuracy, id_muscl, id_visc, nx, ny, nz, nt, np, blocksE, blocksF, blocksG, threadsE, threadsF, threadsG
    ...
    do t2 = 1, np
      do t1 = 1, nt
        ...
        call calc_E<<<blocksE,threadsE>>>(id_accuracy,rho,u,v,w,p,E)
        call calc_F<<<blocksF,threadsF>>>(id_accuracy,rho,u,v,w,p,F)
        call calc_G<<<blocksG,threadsG>>>(id_accuracy,rho,u,v,w,p,G)
