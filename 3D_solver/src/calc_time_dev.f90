...
contains
  subroutine calc_EFG_basic(id_hybrid,dx,xix,dy,etay,Jacobian,Q,mut,E,F,G)
    ...
    if (id_visc == 1 .or. id_turbulence /= 0) then
      call calc_Ev<<<blocksE,threadsE>>>(dx,xix,dy,Jacobian,rho,u,v,w,T,p,mut,E)
      call calc_Fv<<<blocksF,threadsF>>>(dy,etay,dx,Jacobian,rho,u,v,w,T,p,mut,F)
      call calc_Gv<<<blocksG,threadsG>>>(dx,dy,Jacobian,rho,u,v,w,T,p,mut,G)
    endif
    !print *, trim(cudaGetErrorString(cudaGetLastError()))
    stat = cudaDeviceSynchronize()
  end subroutine calc_EFG_basic
  
  subroutine calc_EFG_hybrid(id_hybrid,dx,xix,dy,etay,Jacobian,Q,mut,E_hybrid,F_hybrid,G_hybrid)
    ...
    if (id_visc == 1 .or. id_turbulence /= 0) then
      call calc_Ev<<<blocksE,threadsE>>>(dx,xix,dy,Jacobian,rho,u,v,w,T,p,mut,E_hybrid)
      call calc_Fv<<<blocksF,threadsF>>>(dy,etay,dx,Jacobian,rho,u,v,w,T,p,mut,F_hybrid)
      call calc_Gv<<<blocksG,threadsG>>>(dx,dy,Jacobian,rho,u,v,w,T,p,mut,G_hybrid)
    endif
    stat = cudaDeviceSynchronize()
  ...
  subroutine RungeKutta_3rd(id_RungeKutta,x,dx_cpu,xix_cpu,y,dy_cpu,etay_cpu,z,Jacobian_cpu,Q,Vin)
    ...
    do t2 = 1, np
      do t1 = 1, nt
        call calc_EFG(id_hybrid,dx,xix,dy,etay,Jacobian,Q_d,mut,E,F,G)
        call calc_step1(E,F,G,Q_d,Q2)
        call set_bc(id_accuracy,Q2)

        call calc_EFG(id_hybrid,dx,xix,dy,etay,Jacobian,Q2,mut,E,F,G)
        call calc_step2(E,F,G,Q_d,Q2,Q3)
        call set_bc(id_accuracy,Q3)

        call calc_EFG(id_hybrid,dx,xix,dy,etay,Jacobian,Q3,mut,E,F,G)
  ...
  subroutine RungeKutta_4th(id_RungeKutta,x,dx_cpu,xix_cpu,y,dy_cpu,etay_cpu,z,Jacobian_cpu,Q,Vin)
    ...
    do t2 = 1, np
      do t1 = 1, nt
        ...
        call set_bc(id_accuracy,Qs)

        call calc_EFG(id_hybrid,dx,xix,dy,etay,Jacobian,Qs,mut,E,F,G)
