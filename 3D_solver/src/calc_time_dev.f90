...
contains
  subroutine calc_EFG_Euler(id_visc,nx,ny,nz,dx,dy,dz,Jacobian,QJ,E,F,G,sensor)
  ...
  subroutine calc_EFG_visc(id_visc,nx,ny,nz,dx,dy,dz,Jacobian,QJ,E,F,G,sensor)
  ...
  subroutine calc_EFG_LES(id_visc,nx,ny,nz,dx,dy,dz,Jacobian,QJ,E,F,G,sensor)
    ...
    real(8), intent(out), device                       :: G(nx-accuracy,ny-accuracy,nz-accuracy+1,5)
    real(8), intent(out), dimension(nx,ny,nz), device  :: sensor
    real(8), dimension(nx,ny,nz), device :: rho, u, v, w, p, T, mut, qc2
    integer stat
    call calc_quantities(nx,ny,nz,Jacobian,QJ,rho,u,v,w,p,T)
    mut = 0.d0
    qc2 = 0.d0
    ...
    call calc_Ev_LES<<<blocksE,threadsE,1>>>(nx,ny,nz,dx,dy,dz,rho,u,v,w,T,p,mut,qc2,E)
    call calc_Fv_LES<<<blocksF,threadsF,2>>>(nx,ny,nz,dy,dx,dz,rho,u,v,w,T,p,mut,qc2,F)
    call calc_Gv_LES<<<blocksG,threadsG,3>>>(nx,ny,nz,dx,dy,dz,rho,u,v,w,T,p,mut,qc2,G)
    stat = cudaDeviceSynchronize()
  ...
  subroutine RungeKutta_3rd(id_RungeKutta,myrank,nx,ny,nz,x,dx_cpu,xix_cpu,y,dy_cpu,etay_cpu,z,dz_cpu,zetaz_cpu,Jacobian_cpu,Q)
    ...
    if (myrank == 0) then
      ...
      allocate(dx(nx-1),xix(nx-1),dy(ny-1),etay(ny-1),dz(nz-1),zetaz(nz-1),Jacobian(nx,ny,nz),sensor(nx,ny,nz),Qre(ny,nz,5))
      ...
      call print_vtk(0,nx,ny,nz,x,y,z,Jacobian_cpu,Q,rhomt,pmt,Tmt,Mmt,vmt,mass0,ke0,entropy0)
    ...
    do t2 = 1, np
      if (myrank == 0) then
        do t1 = 1, nt
          ...
          call calc_EFG(id_visc,nx,ny,nz,xix,etay,zetaz,Jacobian,QJ,E,F,G,sensor)
          call nvtxEndRange
          ...
          call calc_EFG(id_visc,nx,ny,nz,xix,etay,zetaz,Jacobian,QJ2,E,F,G,sensor)
          call calc_step2(nx,ny,nz,0.75d0,0.25d0,0.25d0,1.d0,dx,dy,dz,E,F,G,QJ,QJ2,QJ3)
          call set_bc(nx,ny,nz,Jacobian,QJ3,Qre)

          call calc_EFG(id_visc,nx,ny,nz,xix,etay,zetaz,Jacobian,QJ3,E,F,G,sensor)
      ...
      elseif (myrank == 1) then
        ...
        call print_vtk(t2,nx,ny,nz,x,y,z,Jacobian_cpu,Q,rhomt,pmt,Tmt,Mmt,vmt,mass0,ke0,entropy0)
        call nvtxEndRange
      endif
    enddo
    
    if (myrank == 0) then
      deallocate(QJ,QJ2,QJ3,E,F,G,dx,xix,dy,etay,dz,zetaz,Jacobian,sensor,Qre)
    endif
  ...
  subroutine RungeKutta_4th(id_RungeKutta,myrank,nx,ny,nz,x,dx_cpu,xix_cpu,y,dy_cpu,etay_cpu,z,dz_cpu,zetaz_cpu,Jacobian_cpu,Q)
    ...
    do t2 = 1, np
      do t1 = 1, nt
        ...
        if (myrank == 0) then
          if (kind(id_rescale) == 4) then
            Qre_cpu(:,:,:,:) = QJs(nre:nre+9,:,:,:)
            call MPI_ISEND(Qre_cpu, 50*ny*nz, MPI_REAL8, 1, 2, MPI_COMM_WORLD, ireq, ierr)
          endif
          call calc_EFG(id_visc,nx,ny,nz,xix,etay,zetaz,Jacobian,QJs,E,F,G,sensor)
    ...
    if (myrank == 0) then
      deallocate(QJ,QJs,Rs,E,F,G,dx,xix,dy,etay,dz,zetaz,Jacobian,sensor,Qre)
    endif
    deallocate(Qre_cpu,rhomt,pmt,Tmt,Mmt,vmt)
