...
contains
  ...
  attributes(global) subroutine calc_Ev(dx, xix, dy, Jacobian, rho, u, v, w, T, p, mut, E)
  ...
  attributes(global) subroutine calc_Fv(dy, etay, dx, Jacobian, rho, u, v, w, T, p, mut, F)
  ...
  attributes(global) subroutine calc_Gv(dx, dy, Jacobian, rho, u, v, w, T, p, mut, G)
    real(8), intent(in), dimension(nx), device        :: dx
    real(8), intent(in), dimension(ny), device        :: dy
    real(8), intent(in), dimension(nx,ny), device     :: Jacobian
    real(8), intent(in), dimension(nx,ny,nz), device  :: rho, u, v, w, T, p, mut
