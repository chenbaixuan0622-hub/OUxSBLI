...
contains
  subroutine RungeKutta(nx,ny,nz,nt,np,dx,dy,dz,dt,gamma,T0,Cs,Q)
    ...
    real(8) dxi, dyi, dzi, dtdx, dtdy, dtdz, delta
    ! GPU
    integer stat, len
    ...
    do t2 = 1, np
      do t1 = 1, nt
        ...
        if (id_visc == 1 .or. id_turbulence == 1) then
          call calc_step1(nx,ny,nz,dtdx,dtdy,dtdz,E,F,G,Ev,Fv,Gv,Q_d,Q2)
        else
          call calc_step1(nx,ny,nz,dtdx,dtdy,dtdz,E,F,G,Q_d,Q2)
        endif
        ...
        if (id_visc == 1 .or. id_turbulence == 1) then
          call calc_step2(nx,ny,nz,dtdx,dtdy,dtdz,E,F,G,Ev,Fv,Gv,Q_d,Q2,Q3)
        else
          call calc_step2(nx,ny,nz,dtdx,dtdy,dtdz,E,F,G,Q_d,Q2,Q3)
        endif
        ...
        if (id_visc == 1 .or. id_turbulence == 1) then
          call calc_step3(nx,ny,nz,dtdx,dtdy,dtdz,E,F,G,Ev,Fv,Gv,Q3,Q_d)
        else
          call calc_step3(nx,ny,nz,dtdx,dtdy,dtdz,E,F,G,Q3,Q_d)
        endif
