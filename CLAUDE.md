...
* GH200 profiling captures live in `3D_solver_mixed/nsys_ncu_SoA/` (current, post-SoA/post-`calc_div_mp`) and `3D_solver_fltflt/nsys_ncu/`; use these before speculating about kernel performance. Note `3D_solver/nsys_ncu/` no longer exists — `*.ncu-rep` is gitignored, so those captures were never committed.
  * `.ncu-rep` opens locally: `ncu --import <file> --page raw --csv` (per-kernel counters). **But ncu serializes kernels for replay**, so it cannot measure overlap and it makes a partitioned kernel look artificially fast (a 76-SM kernel still gets the whole memory system). Never draw overlap or SM-scaling conclusions from ncu alone.
  * `.nsys-rep` written by the cluster's nsys 2025.x cannot be opened by older local installs; the `.sqlite` export next to it can. Query `CUPTI_ACTIVITY_KIND_KERNEL` joined to `StringIds` for the per-stream busy/idle timeline — that is the only way to see achieved overlap. `3D_solver_mixed/NSTGV/job_miyabi_nsys.sh` produces both.
